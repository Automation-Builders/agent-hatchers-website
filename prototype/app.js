(() => {
  ...
  function hatchScreen(){const settled=state.slots.length&&state.slots.every(Boolean);return `<div class="stage hatch-zone"><span class="eyebrow">Hatching</span><h2>Hatching ${escapeHtml(state.name||'your agent')}…</h2><p>Bringing ${escapeHtml(state.name||'your agent')} to life from your description.</p><div class="hero-hatch"><div class="egg-scene" id="eggScene"><div class="egg-stage">
    <img class="egg-layer egg-pop" src="/hatchy-pop.webp" alt="">
    <div class="egg-lid" aria-hidden="true">
      <img class="lid-piece lp1" src="/egg-shell.webp" alt="" style="--fx:-105px;--fy:175px;--fr:-150deg">
      <img class="lid-piece lp2" src="/egg-shell.webp" alt="" style="--fx:-59px;--fy:200px;--fr:-70deg">
      <img class="lid-piece lp3" src="/egg-shell.webp" alt="" style="--fx:10px;--fy:210px;--fr:40deg">
      <img class="lid-piece lp4" src="/egg-shell.webp" alt="" style="--fx:64px;--fy:193px;--fr:96deg">
      <img class="lid-piece lp5" src="/egg-shell.webp" alt="" style="--fx:108px;--fy:167px;--fr:158deg">
    </div>
    <img class="egg-layer egg-closed" src="/egg-closed.webp" alt="">
    <svg class="egg-crack" viewBox="0 0 620 620" aria-hidden="true"><path class="crackline" pathLength="1" d="M162,270 L170,276 L178,273 L186,262 L194,267 L202,274 L210,294 L218,303 L258,260 L298,382 L378,356 L386,358 L394,360 L402,268 L410,264 L418,260 L426,254 L434,254 L442,253 L450,250 L458,250 L466,251 L474,250" fill="none" stroke="#3b2c14" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/></svg>
    <span class="egg-bubble">Hi, I'm ${escapeHtml(state.name||'Hatchy')}!</span>
  </div></div></div><div class="hatch-actions">${settled?button('Choose a design for '+escapeHtml(state.name||'your agent')+' →','choose'):`<p class="hatch-status">Hatching your designs…</p>`}</div></div>`;}
  ...
  async function generateAgents(){
    ...
    const fetches=[0,1,2].map(i=>fetchPortrait(i).then(img=>{state.slots[i]={status:'ready',image:img};}));
    await sleep(250);const scene=root.querySelector('#eggScene');if(scene)scene.classList.add('hatched');  // exact homepage hatch
    await Promise.all([sleep(4400),...fetches]);   // wait for the hatch + all 3 designs
    celebrate();
