(() => {
  ...
  function chip(){if(state.variant===null)return `<span class="private-pill">Private prototype for ${escapeHtml(company)}</span>`;const inner=state.selectedImage?`<img class="chip-img" src="${escapeHtml(state.selectedImage)}" alt="">`:bot('v'+state.variant+' chip');return `<span class="agent-chip">${inner}<span>${escapeHtml(state.name)}</span></span>`;}
  ...
  function welcome(){return `<div class="stage welcome-grid"><div><span class="eyebrow">Made for ${escapeHtml(company)}</span><h1>Let’s hatch your first agent.</h1><p class="lead">This private experience is tailored to ${escapeHtml(industryLabel.toLowerCase())}. Together, we’ll name your agent, describe how it should look, watch three designs hatch, and pick your favourite.</p><div class="actions">${button('Start hatching →','next')}</div></div><div class="welcome-art" aria-hidden="true"><div class="preview-egg"></div></div></div>`;}
  ...
  function marketScreen(){
    ...
    return `<div class="app">
      ...
      <div class="board" id="board">
        ...
        <section class="board-group g-rec"><div class="group-head"><h3>Recommended for ${escapeHtml(industryLabel)}</h3><span class="count">${ranked.length}</span></div><div class="p-grid">${ranked.map(agentCard).join('')}</div></section>
        <div class="board-empty" id="board-empty" hidden>No profiles match your search.</div>
      </div>
      <div class="board-foot"><span>Private prototype for ${escapeHtml(company)}</span>${button('Start over','reset',true)}</div>
    </div>`;
  }
  ...
  async function generateAgents(){
    ...
    await Promise.all([0,1,2].map(async i=>{
      ...
      const minMs=1500+i*900;const wait=Math.max(0,minMs-(Date.now()-started));if(wait)await sleep(wait);
